...
if (window.location.href.indexOf("nggallery") >= 0) {
				$('.tabs-nav li:nth-child(2)').addClass('active').stop(true,true).show();
			};	
	...
	(function() {
		...
		$tabsNav.each(function() {
			...
			$this.next().children('.tab-content').stop(true,true).hide()
												 .first();
			if (window.location.href.indexOf("nggallery") >= 0) {
				$this.children('li:nth-child(2)').addClass('active').stop(true,true).show();
			};									 
...
// Wait until the document is ready
		...
		// 	);
		// }
		if(document.location.href.indexOf("/cases/?s=") > -1) {
			var costStart = $('#hiddenCostStart').val();
			var costFinish = $('#hiddenCostFinish').val();
			var attendeeStart = $('#hiddenAttendeeStart').val();
			var attendeeFinish = $('#hiddenAttendeeFinish').val();
			var dateStart = $('#hiddenDateStart').val();
			var dateFinish = $('#hiddenDateFinish').val();
			rangeSlider ();
		}else {
			var costStart = 0;
			var costFinish = 2000000;
			var attendeeStart = 0;
			var attendeeFinish = 80000;
			var dateStart = 1980;
			var dateFinish = 2014;
			rangeSlider ();
		}
		...
		function rangeSlider () {
			...
			$('#attendee-range-slider')
				.noUiSlider({
					 range: [0,80000]
					,start: [attendeeStart,attendeeFinish]
					,handles: 2
					,connect: true
					,step: 1000
					,serialization: {
						 to: [ $('#attendee-number-1')
							  ,$('#attendee-number-2') ]
						,resolution: 1
					}
				});
