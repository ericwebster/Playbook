...
if( !function_exists('ss_framework_main_navigation') ) {
	...
	function ss_framework_main_navigation() {
		...
		if ( get_query_var('post_type') == 'portfolio' || get_query_var('taxonomy') == 'portfolio-categories' || is_tag() ) {
...
/* ---------------------------------------------------------------------- */
/*	Add Tag support to custom post types
/* ---------------------------------------------------------------------- */
...
function query_post_type($query) {
  if(is_category() || is_tag()) {
	...
	if($post_type)
	    ...
	    $post_type = array('post','portfolio', 'solutions');
				...
				$output .= '<ul data-effect="' . $slider_effect . '" data-speed="' . $slider_speed . '" data-timeout="' .  $slider_timeout . '" data-mode="' . $disable_project_slider . '">';
					...
					foreach( $project_slider[0] as $i => $slide ) {
						...
						if( $slide['slide-type'] == 'image' ) {
							...
							$attachment_caption = get_post_field('post_excerpt', ss_framework_get_attachment_id( $slide['slide-img-src'] ));
							...
							if( $lightbox == '1' )
								...
								if($attachment_caption){
									$output .= '<div class="" style="padding:15px; background: #efefef;">'. $attachment_caption  .'</div>';
								}
...
if ( !function_exists('ss_framework_portfolio_filter') ) {
	...
	function ss_framework_portfolio_filter() {
		...
		if( ss_framework_has_shortcode('case') ||  ss_framework_has_shortcode('solution') || is_search() ) {
			...
			if( is_tax('portfolio-categories') || is_search() ) {
			...
			$categories = get_terms('portfolio-categories', $top_level_args);
			...
			foreach ( $categories as $category ) {
				...
				if( isset( $portfolio_categories ) && in_array( $category->term_id, $portfolio_categories ) || !isset( $portfolio_categories ) )
					$output .= '<li' . $current_item . '><a href="' . get_term_link( $category->slug, 'portfolio-categories' ) . '" data-categories="' . $category->slug . '">' . $category->name . '</a>';
...
if ( !function_exists('ss_framework_search_filter') ) {

	function ss_framework_search_filter() {
		...
		if( is_search() ) {
			...
			$shortcode_categories = get_post_types();
		...
		$custom_post_types = get_post_types( array(
				...
				$current_category = $wp_query->get_queried_object( get_post_type($post));
			...
			$categories = get_post_types( $custom_post_types , 'names' );
			$query = get_search_query();
			
			$output = '<ul id="portfolio-items-filter" class="' . ( is_tax('biography-categories') ? 'single-category' : '' ) . ' ' . ( of_get_option('ss_portfolio_category_filter') ? 'open' : '' ) . '">';

			$output .= '<li>' . __('Showing', 'ss_framework') . '</li>';

				if ($_SERVER['REQUEST_URI'] == '/?s=' . $query . '&submit=Search') {
					$currentAll = 'current';
				}

			$output .= '<li class="all '. $currentAll .' "><a data-categories="*" href="?s=' . $query . '&submit=Search">' . __('All', 'ss_framework') . '</a></li>';
			...
			if ($_SERVER['REQUEST_URI'] == "/?post_type=".  $post_type . "&s=".  $query . "&submit=Search") {
				...
				if( isset( $portfolio_categories ) && in_array( $category->term_id, $portfolio_categories ) || !isset( $portfolio_categories ) )
					...
					$output .= '<li class="' . $current_item . $post_type . '"><a href="?post_type=' .  $post_type . '&s=' .  $query . '&submit=Search" data-categories="' .  $post_type . '">' . $post_type_name . '</a>';
	...
	add_action('ss_framework_search_filter', 'ss_framework_search_filter');
